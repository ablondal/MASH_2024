...
int main() {
    ...
    vector<vector<bool>> visited_in_line(n, vector<bool>(m));
    ...
    while(!coords.empty()) {
        ...
        while(!coords.empty()){
            ...
            for(int newRow = row - 1; newRow >= 0; newRow--){
                if(visited_in_line[newRow][col] || maze[newRow][col] == '#'){
                    break;
                }
                visited_in_line[newRow][col] = true;
            ...
            for(int newCol = col - 1; newCol >= 0; newCol--){
                if(visited_in_line[row][newCol] || maze[row][newCol] == '#'){
                    break;
                }
                visited_in_line[row][newCol] = true;
